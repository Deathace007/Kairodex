...
# Same hardcoded, DST-naive approximate session windows as
# `kairodex.risk.loader.is_session_open` — no `trading_calendar` data is
# wired into this path either (same documented gap). Good enough to
# bucket "which third of the session did this trade open in," not a
# substitute for the real calendar.
_SESSION_WINDOWS: dict[Market, tuple[datetime.time, datetime.time]] = {
    Market.NSE: (datetime.time(3, 45), datetime.time(10, 0)),
    Market.US: (datetime.time(13, 30), datetime.time(20, 0)),
}
...
def _session_bucket(t: TradeRecord) -> str | None:
    open_t, close_t = _SESSION_WINDOWS[t.segment.market]
    open_dt = datetime.datetime.combine(t.opened_at.date(), open_t, tzinfo=datetime.UTC)
    close_dt = datetime.datetime.combine(t.opened_at.date(), close_t, tzinfo=datetime.UTC)
